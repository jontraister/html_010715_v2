		...
		_continueToNextStep : function(){
			if(this.isLastStep){
				...
				if(!this.options.formPluginEnabled){
					$(this.element).trigger('formEnd', this._state());
					return true;
				}else{
		...
		_getLink : function(step){
			...
			if(links != undefined){
				if(links.filter(":radio,:checkbox").size() > 0){
					link = links.filter(this.options.linkClass + ":checked");

					if(link.size() > 0){
						var m= $(link).attr("class").match(/link\-([^ ]+)/);
					...
					if(links.filter("select").size()>0){
						if(links.find("option:selected").size()>0){
							
							var option = links.find("option:selected").eq(0);
							
							if(option.attr("class")!==undefined){
								link = option.attr("class");
							}
							else {
								link = option.val();
							}
						...
						if(links.length > 0){
							var m= $(links).attr("class").match(/link\-([^ ]+)/);
							if(m!==null && m[1]!==undefined){
								link = m[1];
							}
							else {
